...
const state={stage:"setup",bag:"",ppp:"",shift:"",categoryIndex:0,quantities:{},notes:"",startMode:"new",builtBag:"",saving:false,saveResult:null};

function toast(message){ const el=document.querySelector("#toast"); el.textContent=message; el.classList.add("show"); setTimeout(()=>el.classList.remove("show"),2200); }
function esc(value=""){ return String(value).replace(/[&<>'"]/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;","'":"&#39;",'"':"&quot;"}[c])); }
function shiftSwitchHtml(){ return `<select class="shift-inline-select" data-shift-switch aria-label="Tukar shift">${SHIFTS.map(shift=>`<option value="${shift}" ${shift===state.shift?"selected":""}>${shift}</option>`).join("")}</select>`; }
function buildQuantities(mode){
  ...
  state.startMode=mode; state.builtBag=state.bag; state.quantities={};
  categories.forEach(category=>{
    const previous=mode==="copy"?last?.quantities?.[category.id]?.items:null;
    state.quantities[category.id]={items:category.items.map(([name,standard])=>{
      const match=previous?.find(item=>normalizeKey(item.name)===normalizeKey(name));
      const hasQty=match&&match.qty!==undefined&&match.qty!==null&&match.qty!=="";
      return {name,standard,qty:hasQty?Math.min(standard,Number(match.qty)):standard};
    })};
  });
}
...
root.addEventListener("click",async event=>{
  ...
  const startButton=event.target.closest("[data-start-mode]"); if(startButton){ state.ppp=document.querySelector("#pppName").value.trim(); if(!state.ppp){ document.querySelector("#pppName").focus(); toast("Masukkan nama PPP dahulu."); return; } const mode=startButton.dataset.startMode; const alreadyBuilt=state.builtBag===state.bag&&state.startMode===mode&&Object.keys(state.quantities).length>0; if(!alreadyBuilt) buildQuantities(mode); state.stage=mode==="copy"?"review":"category"; render(); return; }
...
root.addEventListener("change",event=>{
  const shiftSwitch=event.target.closest("[data-shift-switch]");
  if(shiftSwitch){ state.shift=shiftSwitch.value; }
});
